...
| `version_info.txt` (File/ProductVersion) | `0.7.0` / `(0, 7, 0, 0)` |
| `installer/praatMaar.iss` fallback `#define MyAppVersion` | `0.7.0` |
| `scripts/build-windows.ps1` default `-Version` | `0.7.0` |
| Git-tag | `v0.7.0` |
| `CHANGELOG.md` | sectie `[0.7.0] - YYYY-MM-DD`, `[Unreleased]` leegmaken |

Huidige **gepubliceerde** release: **v0.6.0**. Deze branch cut **v0.7.0**
(CHANGELOG-sectie `[0.7.0]`); tag na merge + bevestiging.
...
3. Alles in één keer (versie expliciet meegeven):
   ...
   ```powershell
   .\scripts\build-windows.ps1 -Version 0.7.0
   ```

   Of alleen de app-map:

   ```powershell
   .\scripts\build-windows.ps1 -Version 0.7.0 -SkipInstaller
   ```
